...
(h) Parse the subagent's JSON response. Extract `status` and `suggested_next_needs`.
  ...
  - **No-op pass-through rule (B022).** `nothing_to_test` and `nothing_to_review` are completed applicability decisions, not failures or incomplete work. Their workers return the next discipline and explain the evidence in `notes`. Apply that non-null transition so an inapplicable discipline does not repeatedly redispatch the same entry and starve later work. `cannot_proceed` with a null transition remains the hold path.
