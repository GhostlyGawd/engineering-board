...
| Codex adapter | A repository-owned plugin manifest packages the five skills, the isolated 19-tool MCP configuration, and an explicit empty Codex hook source. | Codex does not load the Claude Code hook workflow. It uses the MCP-first workflow and requires an explicit repository root for each bundled-plugin tool call. |
...
| Codex plugin | Accepted current direction | `.codex-plugin/plugin.json` packages five MCP-first skills, selects an explicit empty Codex hook source, and starts only Engineering Board through the root `.mcp.json` launcher. Each tool call requires the repository root. Claim and release use the Python server instead of Claude hook scripts | Codex plugin host-boundary and launcher test, MCP lifecycle suite, plugin validator, and dated installation evidence | README, MCP reference, architecture, security, release procedure, changelog, skills, and this central spec aligned in the same change | Published in v1.13.0; immutable marketplace pin corrected in v1.13.1; explicit host-boundary correction pending release |
