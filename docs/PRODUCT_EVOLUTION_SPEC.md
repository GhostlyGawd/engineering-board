...
_Current release boundary: `v1.13.3`_
...
|---|---|---|
| Finding capture | Claude Code Stop capture and explicit MCP capture append evidence-backed scratch findings. Foreground promotion previews an unchanged, content-bound plan before it writes canonical entries. | Scratch does not participate in graph memory until explicit promotion. Automatic capture remains host-specific and never grants belief authority. |
| Canonical memory | Readable Markdown stores entries, P### patterns, H### hypotheses and outcome history, and L### Learnings. | `BOARD.md`, `GRAPH.yml`, context briefs, reports, cache, JSON, and HTML remain derived and must be rebuildable from canonical sources. |
| Pattern vocabulary | Repository-owned P### records preserve readable labels, aliases, definitions, inclusion evidence, and exclusions. Preview/apply operations assign or correct identity while retaining legacy tags as evidence. | Semantic suggestions cannot silently merge or replace canonical identities. Unresolved raw labels remain visible until an explicit correction. |
| Structural graph | One zero-dependency shared core builds deterministic nodes, source-linked edges, topology, clusters, and typed findings for plugin, MCP, and contained-demo adapters. | The graph connects declared canonical evidence. It does not treat lexical or semantic similarity as a relationship fact or causal proof. |
| Cluster and hypothesis surfacing | `board_insights` exposes recurrence, domain diversity, severity, recency, evidence quality, linked H### states, and rejected negative memory. | The score prioritizes investigation. It does not confirm a cause; only explicit investigation or fix evidence can change H### authority. |
| Learning curation | Source-linked Learnings retain recurrence, confidence, outcome state, and structured outcome feedback. Explicit remember and accepted deterministic curation are separate write paths. | Curation and confidence changes require named evidence and explicit mutation authority. Recurrence alone cannot override a failed or contested outcome. |
| Context retrieval | Context contract version 3 deterministically retrieves bounded clusters, hypotheses, negative memory, and matched Learnings from canonical patterns, affected paths, graph proximity, selected entries, and current directory. | Task terms refine eligible memory but cannot create structural eligibility. The latest locked D.1 run did not prove the required diagnosis effect. |
| Resolution feedback | `board_outcomes` records held, failed, partial, or inconclusive fix results against H### records and previews separate Learning feedback. | The system does not infer an outcome from closure or silently confirm, weaken, reject, or resolve memory. Each mutation requires explicit evidence and unchanged apply authority. |
| Quality workflow | Worker mode can drive the optional `tdd → review → validate` loop, retain claims, and request explicit resolution. | The loop verifies a correction and can supply outcome evidence. It remains downstream support, not the product identity or a required first experience. |
| Codex adapter | A repository-owned plugin manifest packages the five skills, a Codex-specific 19-tool MCP configuration with a `writes` approval default, and an explicit empty Codex hook source. | Six pure-read tools can run without a per-call prompt. Every write-capable tool remains gated. Codex does not load the Claude Code hook workflow and requires an explicit repository root for each bundled-plugin tool call. |
...
| MCP behavior | Required current truth | Nineteen tools share canonical Markdown and publish explicit maximum-capability annotations. Pattern, promotion, graph, ranking, context, hypothesis, outcome, and Learning tools delegate to the same core as plugin adapters | Exact tools/list annotation contract, MCP lifecycle suite, and Milestone B-D adapter-parity matrices | MCP reference, manifests, package modules, counts, approval behavior, and advisory-hint boundary updated | Core behavior published in v1.11.0; annotation correction pending release |
| Codex plugin | Accepted current direction | `.codex-plugin/plugin.json` packages five MCP-first skills, selects an explicit empty Codex hook source, and starts only Engineering Board through `codex-mcp.json`. That file sets the `writes` approval default. Each tool call requires the repository root. Claim and release use the Python server instead of Claude hook scripts | Codex plugin host-boundary and launcher test, controlled annotated/unannotated host comparison, MCP lifecycle suite, plugin validator, and dated installation evidence | README, MCP reference, architecture, security, release procedure, changelog, skills, and this central spec aligned in the same change | Published in v1.13.0; immutable marketplace pin corrected in v1.13.1; explicit hook boundary corrected in v1.13.2; annotation correction pending release |
...
The MCP schema uses maximum-capability annotations. A tool that only reads
canonical or derived repository state sets `readOnlyHint: true`. A tool that
can write sets it to false, even when one action in that tool is a read-only
list, report, or preview. Each tool also declares destructive, idempotent, and
open-world hints. All Engineering Board tools are closed-world because they
operate only on the caller-selected local repository.

The Codex plugin uses the `writes` approval mode. Pure reads can run without a
per-call prompt. Every write-capable tool remains gated. Annotations are
advisory and untrusted; they never grant write authority or replace host
policy, path containment, content-bound plans, claim ownership, or apply-time
revalidation.
