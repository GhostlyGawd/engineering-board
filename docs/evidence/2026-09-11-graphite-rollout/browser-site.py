...
BASE=os.environ.get('BASE_URL','http://127.0.0.1:51330/')
    ...
    for width,height in [(1448,1086),(390,844)]:
      for theme in ['dark','light']:
        ...
        for id in ['B001','B002','B003','H001']:
          ...
          assert page.url.endswith(f'/example/{id}.html')
          ...
          assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
          if width==390 and id=='B002': page.screenshot(path=str(OUT/f'site-mobile-source-{theme}.png'),full_page=True)
        ...
        page.screenshot(path=str(OUT/f'site-{width}-{theme}.png'),full_page=True)
      ...
      for id in ['B001','B002','B003','H001']:
        ...
        page.get_by_role('link',name='Back to finding' if id!='H001' else 'Back to investigation',exact=True).click()
        assert page.locator(f'#entry-{id}').is_visible()
      page.screenshot(path=str(OUT/f'site-{width}-nojs.png'),full_page=True)
      ...
      c.close()
    browser.close()
(OUT/'site-results.json').write_text(json.dumps(results,indent=2))
