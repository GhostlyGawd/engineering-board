...
1. **§6.6 prose** — README/ARCHITECTURE trees, skills (4), agents (8), `references/`, and the two router-path refs in `hooks/stop-hook-procedure.md` (**edit path strings only — pinned tokens are guarded by `tests/modes/stop-hook-mode-routing.sh`**).
2. **§8 fixtures** — repoint the ~19 fixtures (+5 adversarial), but **deliberately keep a few on `docs/boards/` + `docs/board/`** to guard the fallback.
