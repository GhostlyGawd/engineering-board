...
Current release line: **v1.13.1**. Canonical evidence, P### pattern
...
| `board-resolve` | "close this", "mark resolved", "question answered" | (bug/feature) verify done-when to set resolved to newest-first ARCHIVE insertion to `/board-rebuild` to auto-resolve cascade. (question) write Finding FIRST to set resolved to newest-first ARCHIVE insertion to unblock dependents to auto-resolve cascade to triage. (observation) set resolved to newest-first ARCHIVE insertion to `/board-rebuild` to auto-resolve cascade | entry + ARCHIVE.md + dependent unblocks |
