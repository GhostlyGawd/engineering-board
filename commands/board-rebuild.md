...
Sort:
1. Bugs by priority (P0 to P1 to P2 to P3 to unranked), then by ID ascending within a priority
...
Report:
- Lines added (entries that should be in BOARD.md but were not)
...
**Why at rebuild:** rebuild is the canonical "I'm refreshing my picture of board state" command. It already detects drift between filesystem and BOARD.md. The auto-resolve pass detects the related drift between filesystem and *truth*: entries that should be closed but are not.
...
**Silent path:** zero candidates to no output, proceed to Step 9.

**If the user closes any entries from the pass:** re-run Steps 4-7 for that board to re-emit BOARD.md and GRAPH.yml with the closures reflected. The second pass is idempotent: will not loop because the closed entries are no longer at `status: open`.
...
- **What this does NOT do**: rebuild ARCHIVE.md (resolutions need provenance and timestamps that are not preserved in a derived view). modify entry files except via the explicit Step 8 auto-resolve pass (which prompts before any close). silently auto-close anything: every closure flows through user confirmation in Step 8.
