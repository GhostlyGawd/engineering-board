...
3. Back-fills `needs: tdd` on bug/feature entries with `status: open` (or `in_progress`, `blocked`) that do not already have a `needs:` field. Resolved entries are left alone.
...
2. Moves `docs/boards/<project>/` to `engineering-board/<project>/` (prefers `git mv` inside a work tree, so the move is history-preserving and reversible. falls back to plain `mv`).
...
## Relocate (1.1.0): docs/boards/ to engineering-board/
...
After a successful relocate, run `/board-rebuild` so `BOARD.md`/`GRAPH.yml` reflect the new paths, and remind the user to add the `engineering-board/` runtime stanza to `.gitignore` (printed by `/board-init`) if they have not already.
