...
Read the entry's `## Done when` section. Confirm the stated verification has been performed. If criteria are not met, do not proceed: note what remains.
...
After the cascade pass settles, apply the board-triage rules to the now-current open items and state the recommended next step. Do not wait to be asked. (Triage itself will run its own auto-resolve pass: that is expected and idempotent. the cascade pass at Step 8 will already have surfaced anything triage's pass would catch, so triage's pass should be a no-op in the typical case.)
