...
- **v1.0.1** Scratch-append fidelity (issue #3) — new `board-scratch-append.sh` owns the scratch write: it computes the `<!-- iso8601 -->` timestamp, validates the finding shape, re-serializes canonically, and atomically appends. Stop-procedure step (d) now pipes the finding-extractor's JSON into it through a quoted heredoc, removing the orchestrating LLM from the byte-copy path that let a `printf`/`echo` hop mangle `evidence_quote` and silently break anchor verification. A malformed copy now fails loudly instead of deferring without a trace. Pinned by `tests/scratch/append.sh` (13 assertions, incl. an end-to-end consolidate-promotion check); `tests/run-all.sh` is now 9 suites.
