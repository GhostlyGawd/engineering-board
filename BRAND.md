...
- Headings: semibold/bold, tight tracking (`-0.02em`). Eyebrow labels: uppercase,
  ...
  (`--eb-fs-xs` to `--eb-fs-4xl`) in `tokens.css`.
...
- Do not stretch, rotate, add shadows/glows, or outline the wordmark text.
...
- **Do not:** hype words (revolutionary, magical, seamless, blazing-fast,
  game-changing), exclamation marks, or claims we cannot back with a feature and a
